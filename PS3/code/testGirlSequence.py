...
rect = [280, 152, 330, 318] # initial rectangle value
...
def show_frame(f, save=False):
    ...
    if save:
        plt.axis('off')
        plt.savefig("../outputs/q1_3_girl_{}.png".format(f), bbox_inches = 'tight')
    else:
        plt.show()
...
np.save('../result/girlseqrects.npy', rects) # Note: rect is allowed to be float according to Piazza @323
