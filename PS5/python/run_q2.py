...
for itr in range(max_iters):
    ...
    for xb,yb in batches:
        ...
        total_loss += loss
        avg_acc += acc # sum up now and divide by number of batches at end to get avg.
...
# Rerun forward+backprop to determine post-training gradients (per @551_f1 on Piazza):
h1 = forward(x,params,'layer1')
probs = forward(h1,params,'output',softmax)
...
for k,v in params.items():
    ...
    params['grad_'+k] = np.zeros(params[k].shape)
    # loop over every value in the parameter:
    for i in range(np.size(params[k])):
        # Using the center diff (as described in the assignment so this runs
        # twice to get J for both +eps and -eps)
        original = params[k].ravel()[i]
        
        # add eps:
        params[k].ravel()[i] = original + eps
        # run the network:
        h1 = forward(x,params,'layer1')
        probs = forward(h1,params,'output',softmax)
        # get the loss:
        loss_upper, _ = compute_loss_and_acc(y, probs)
        ...
        probs = forward(h1,params,'output',softmax)
        # get the loss:
        loss_lower, _ = compute_loss_and_acc(y, probs)
       
        # reset param:
        params[k].ravel()[i] = original
        
        params['grad_'+k].ravel()[i] = (loss_upper - loss_lower) / (2*eps)
